...
<body style="background: #edf2f7;">
      ...
      <div class="py-3 ">
            ...
            <div class="flex space-x-8 w-full">
               ...
               <? } else { ?>
                  <div id="" class="h-48 overflow-visible w-1/4">
                     ...
                     <img class="transform transition duration-1000 ease-in-out hover:scale-110 hover:skew-x-2 rounded-3xl shadow-2xl cursor-pointer" src='./poster/<?php echo $posters['percorso_immagine']; ?>' alt="" onclick="showModal('./poster/<?php echo $posters['percorso_immagine']; ?>')">
                  </div>
               <? } ?>
               <!--POSTERS-->
               <div class="flex flex-col w-full max-w-fit">
                  <div class="flex justify-between items-start">
                     ...
                     <? $vote_color = print_vote($row['media']); ?>
                     <div data-popover-target="chart" data-popover-placement="left" class="transform transition duration-500 hover:scale-150 absolute right-20 top-28 bg-<? echo $vote_color; ?>-500 text-white text-xl font-bold rounded-full px-3 py-3.5 hover:bg-opacity-0 hover:font-extrabold hover:text-<? echo $vote_color; ?>-500"><? echo $row['media'] + 0; ?></div>
                  ...
                  <div class="flex space-x-1 mb-10">
                     ...
                     <div class="text-white font-medium text-lg">
                        ...
                        if ($row['tipo'] == 0) {
                           ...
                           echo $row['uscita'] . " - " . $row['durata'];
                        } else {
                           echo $row['n_stagioni'] . " stagioni - " . $row['inizio'] . "/";
                           if ($row['fine'] == 0) {
                              echo "In corso";
                           } else echo $row['fine'];
                        }
                  ...
                  <button button data-popover-target="popover" data-popover-placement="right" name="id" value="' . $id . '" class=" bg-' . $colore . '-600 shadow-lg text-white  p-2 rounded  hover:bg-' . $colore . '-500 hover:text-gray-100 transform transition-all duration-700 ease-in-out hover:scale-110">Modifica la tua recensione </button>
               ...
               <input type="hidden" id="id" name="id" value="' . $review_id . '" />
               <button type="submit" class=" bg-' . $colore . '-600 shadow-lg text-white  p-2 rounded  hover:bg-red-600 hover:text-gray-100 transform transition-all duration-700 ease-in-out hover:scale-110">Cancella la tua recensione</button>
               </form>
               ';
                        } else {
                           echo
                           '<form action="/write_review.php" method="get">
                        <button button data-popover-target="popover" data-popover-placement="right" name="id" value="' . $id . '" class=" bg-' . $colore . '-600 shadow-lg text-white  p-2 rounded  hover:bg-' . $colore . '-500 hover:text-gray-100 transform transition-all duration-700 ease-in-out hover:scale-110">Scrivi
                     ...
                     <button type="submit" class=" bg-' . $colore . '-600 shadow-lg text-white  p-2 rounded  hover:bg-' . $colore . '-500 hover:text-gray-100 transform transition-all duration-700 ease-in-out hover:scale-110">Modifica scheda</button>
                  </form>';
                     }
                     ?>
                     <? if ($seen == 2) {
                     ...
                     <input type="hidden" id="id" name="id" value="' . $row['id'] . '" />
                     <button type="submit" class=" bg-' . $colore . '-600 shadow-lg text-white  p-2 rounded  hover:bg-' . $colore . '-500 hover:text-gray-100 transform transition-all duration-700 ease-in-out hover:scale-110">Aggiungi alla watchlist</button>
                  </form>';
                     } elseif ($seen == 0) {
                        echo
                        '<form action="./db/mark_as_watched.php" method="post">
                     <input type="hidden" id="id" name="id" value="' . $row['id'] . '" />
                     <button type="submit" class=" bg-' . $colore . '-600 shadow-lg text-white  p-2 rounded  hover:bg-' . $colore . '-500 hover:text-gray-100 transform transition-all duration-700 ease-in-out hover:scale-110 ">Segna come visto</button>
                  </form>';
                     } elseif ($seen == 1) {
                        echo
                        '<form action="" method="post">
                     <button disabled type="submit" class=" bg-' . $colore . '-600 shadow-lg text-white  p-2 rounded  hover:bg-' . $colore . '-500 hover:text-gray-100">Visto!</button>
                  </form>';
                     }
                     ?>
                  </div>
            ...
            <div class=" flex gap-1 overflow-x-scroll scrollbar-hide overflow-y-visible h-72 py-2 px-1 mt-1 w-4/6 bg-gradient-to-r from-<? echo $colore ?>-900 to-<? echo $colore ?>-500 shadow-lg mx-2 rounded-lg border-gray-100 border">
               ...
               foreach ($cast as $member) {
                  echo '<form action="./search_result.php" method="get">
                     ...
                     <div onclick="javascript:this.parentNode.submit();" class="cursor-pointer transform transition duration-500 hover:scale-105 h-64 w-36 min-w-36 m-1 bg-white rounded-lg flex flex-col shadow-sm hover:shadow-2xl hover:shadow-slate-400 ease-in-out">
            ...
            <div class="h-72 p-3 ml-1 w-2/6 mr-2 mt-1 overflow-clip flex items-center justify-center bg-gradient-to-r from-<? echo $colore ?>-900 to-<? echo $colore ?>-500 shadow-lg rounded-lg border border-gray-100">
               <? if ($trailers->num_rows > 1) { ?>
                  <div id="controls-carousel" class="relative w-full" data-carousel="static">
                     ...
                     <div class="relative h-64 overflow-hidden rounded-lg">
                        ...
                        <? foreach ($trailers as $trailer) { ?>
                           <div class="hidden duration-700 ease-in-out" data-carousel-item>
                              <iframe class="aspect-video mt-1 rounded-lg w-trailer-custom" src="<?php echo $trailer['link']; ?>" allowfullscreen>
                              </iframe>
                           </div>
                        <? } ?>
                     </div>
                     <!-- Slider controls -->
                     <button type="button" class="absolute top-20 left-0 z-30 flex items-center justify-center h-1/3 px-4 cursor-pointer group focus:outline-none" data-carousel-prev>
                     ...
                     <button type="button" class="absolute top-20 right-0 z-30 flex items-center justify-center h-1/3 px-4 cursor-pointer group focus:outline-none" data-carousel-next>
                        <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
                           <svg aria-hidden="true" class="w-6 h-6 text-white dark:text-gray-800" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
               ...
               <? } else {
                  ...
                  <iframe class="aspect-video rounded-lg w-trailer-custom" src="<?php echo $trailers['link']; ?>" allowfullscreen>
                  </iframe>
               <? } ?>
            </div>
         </div>
         ...
         <div class="grid place-items-center">
            <ul class="my-2 space-y-4">
               <li class="opacity-0 transition-all duration-1000 ease-in w-custom flex mt-1">
               <div class="text-3xl">Recensioni pubblicate  -  Ordina per </div>
               <button type="button" onclick="location.href='./film.php?id=<?echo $row['id']?>'" class="bg-blue-600 text-white  p-2 ml-2 rounded-xl  hover:bg-blue-500 hover:text-gray-100">Data</button>
            <button type="button" onclick="location.href='./film.php?id=<?echo $row['id']?>&ord=1'" class="bg-blue-600 text-white  p-2 mx-2 rounded-xl  hover:bg-blue-500 hover:text-gray-100">Mi piace</button>
            <button type="button" onclick="location.href='./film.php?id=<?echo $row['id']?>&ord=2'" class="bg-blue-600 text-white  p-2 rounded-xl  hover:bg-blue-500 hover:text-gray-100">Voto</button>
               ...
               foreach ($reviews as $review) {
                  ...
                  echo  '<li class="opacity-0 transition-all duration-1000 ease-in transform hover:scale-105 bg-white border border-gray-100 w-custom p-4 rounded-lg shadow-sm hover:shadow-2xl">
                        <div class="flex justify-between items-start">
                           <div class="flex">
                              <div class="text-gray-600 py-3 text-lg">Recensione di</div>
                              <div onclick="location.href = \'./profile.php?usr=' . $review['username'] . '\';" class="transform transition hover:scale-105 text-blue-500 ml-1 py-3 text-lg hover:underline cursor-pointer">' . $review['username'] . '</div>
                  ...
                  echo '<div class="transform transition duration-500 hover:scale-150  bg-' . $vote_color . '-500 text-white font-bold rounded-full px-3.5 py-3 hover:bg-white hover:text-' . $vote_color . '-500">
                  ' . $review['voto'] . '</div>';
                  echo '</div>
                  ...
                  if (in_array($review['id'], $likes)) {
                     echo '<button onclick="unlike(this.id, this.value,\'' . $_SESSION['username'] . '\')" id="' . $likebutton . '" value="' . $review['id'] . '" class=" transition-colors duration-700 ease-in-out hover:bg-red-600 hover:text-white h-10 w-40 hidden cursor-pointer rounded-lg border-2 border-red-600 py-1 px-2 font-bold text-red-600">Mi piace: ' . $review['count'] . '</button>
                           <button onclick="unlike(this.id, this.value,\'' . $_SESSION['username'] . '\')" id="' . 'unlike' . $likebutton . '" value="' . $review['id'] . '" class="transition-colors duration-700 ease-in-out hover:bg-white hover:text-red-600 h-10 w-40 cursor-pointer rounded-lg border-2 bg-red-600 border-red-600 py-1 px-2 font-bold text-white">Mi piace: ' . $review['count'] . '</button>';
                  } else {
                     echo '<button onclick="like(this.id, this.value,\'' . $_SESSION['username'] . '\')" id="' . $likebutton . '" value="' . $review['id'] . '" class="transition-colors duration-700 ease-in-out hover:bg-red-600 hover:text-white h-10 w-40 cursor-pointer rounded-lg border-2 border-red-600 py-1 px-2 font-bold text-red-600">Mi piace: ' . $review['count'] . '</button>
                        <button onclick="like(this.id, this.value,\'' . $_SESSION['username'] . '\')" id="' . 'unlike' . $likebutton . '" value="' . $review['id'] . '" class="transition-colors duration-700 ease-in-out hover:bg-white hover:text-red-600 h-10 w-40 hidden cursor-pointer rounded-lg border-2 bg-red-600 border-red-600 py-1 px-2 font-bold text-white">Mi piace: ' . $review['count'] . '</button>';
                  }
                  ...
                  foreach ($review_tags as $tag) {
                     echo '<div onclick="location.href = \'./search_result.php?search_tag=' . $tag['label'] . '\';" class="transform transition hover:scale-105 bg-white cursor-pointer rounded-lg w-fit p-1 shadow-2xl h-8">
                     <div class="text-black text-xs">' . $tag['label'] . '</div>
                  </div>';
                  }
                  echo '</div>
                  ...
                  if ($comments->num_rows != 0) {
                     ...
                     foreach ($comments as $comment) {
                        ...
                        echo '<li class="mt-2 bg-white border border-gray-100 w-full p-4 rounded-lg shadow-sm hover:shadow-2xl transition-shadow duration-300 ease-in-out">
                        ...
                        if ($_SESSION['tipo'] > 0 or $_SESSION['username'] == $comment['username']) {
                           ...
                           echo '<li>
                                       <form action="./db/cancel_comment.php" method="post">
                                          ...
                                          <input type="hidden" id="id_scheda" name="id_scheda" value="' . $row['id'] . '" />
                                          <a onclick="javascript:this.parentNode.submit();" class="cursor-pointer block z-50 px-4 py-2 hover:bg-gray-100">Cancella</a>
                                       </form>
                                       </li>
                                       </ul>';
                        }
   ...
   <script>
      ...
      // Funzione per gestire l'intersezione dell'osservatore
      ...
      const observer = new IntersectionObserver(handleIntersection, {
         root: null,
         rootMargin: '0px',
         threshold: 0.7, // Modifica questo valore se necessario
      });

      // Seleziona tutti gli elementi <li> delle recensioni
      const reviewLists = document.querySelectorAll('li');

      // Osserva ciascun elemento <li> delle recensioni
