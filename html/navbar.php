         ...
         <div class=" z-40 overscroll-none fixed top-0 w-screen flex flex-row items-center p-1 justify-between bg-white shadow-xl">
            <div class="cursor-pointer ml-8 text-lg text-gray-700 hidden md:flex" onclick="location.href='./dashboard.php'">MovieConnect</div>
               ...
               <div class="flex mr-8 md:flex">
                  <?if($_SESSION['tipo'] == 2){//Se l'utente è un admin può accedere al pannello dello staff?>
                  ...
                  <?if($_SESSION['tipo'] == 1){//Se l'utente è un moderatore può accedere al pannello delle richieste di inserimento?>
                     <button type="button" onclick="location.href='./requests.php'" class="bg-red-600 text-white mr-2 p-2 rounded  hover:bg-red-500 hover:text-gray-100">Richieste di inserimento</button><?}?>
